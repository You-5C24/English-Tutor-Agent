...
**安全机制**：循环次数上限（`MAX_TOOL_ITERATIONS`）仍然保留——在 `executeTools` 节点中计数，达到上限时强制路由到 `respond`。
...
### Phase 4：Streaming 接入（为后续 Phase 4 产品功能铺路）

**目标**：验证改造后的图支持 `graph.stream()` / `graph.streamEvents()`，输出技术验证报告。**不实现前端 Streaming UI**——那是产品层面的 Phase 4。
