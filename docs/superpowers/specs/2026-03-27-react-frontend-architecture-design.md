...
**非目标（Phase 1 不做）：** 多会话管理、消息持久化、Markdown 渲染、流式响应、TTS、数字人。
...
- `sessionId` 暴露 + `resetSession()` — Phase 2 多会话
...
Phase 1（MVP 文字聊天）← 本次实现
  ├── Phase 2（多会话 + 持久化）— 独立于 P3
  ├── Phase 3（Markdown 渲染）— 独立于 P2
  └── Phase 4（Streaming 响应）
        └── Phase 5（TTS 语音）
              └── Phase 6（数字人）
```

Phase 2、3 互不依赖，完成 Phase 1 后可按任意顺序实施。Phase 4 → 5 → 6 为强依赖链。
